...
export default function Cadastro({ route, navigation }) {
  // Usa a lista de crianças recebida
  ...
  function telaCadastrarCrianca() {
    ...
    // Adicione um ID único para cada criança
    const novaCrianca = new Crianca(nomeCrianca, dataNascimento);
    novaCrianca.vacinas = Todasvacinas;
    novaCrianca.id = Date.now(); // Gera um id único
    adicionarCrianca(novaCrianca);

    setMessage(`${nomeCrianca} cadastrada com sucesso!`);
    setNomeCrianca("");
    setDataNascimento("");
    navigation.navigate("Homestack", {
      screen: "ListaCriancas",
    });
    // Navega para a lista de crianças passando a lista completa atualizada
  }
